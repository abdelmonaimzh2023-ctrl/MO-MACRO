...
public class SettingsActivity extends AppCompatActivity {
    ...
    private void buildUI() {
        ...
        root.addView(label("Start X"));
        etHoldStartX = edit(String.valueOf(prefs.getInt("hold_x1", 540)));
        root.addView(etHoldStartX);
        root.addView(label("Start Y"));
        etHoldStartY = edit(String.valueOf(prefs.getInt("hold_y1", 1200)));
        root.addView(etHoldStartY);
        root.addView(label("End X"));
        etHoldEndX = edit(String.valueOf(prefs.getInt("hold_x2", 540)));
        root.addView(etHoldEndX);
        root.addView(label("End Y"));
        etHoldEndY = edit(String.valueOf(prefs.getInt("hold_y2", 600)));
        root.addView(etHoldEndY);
        root.addView(label("Duration (ms)"));
        etHoldDur = edit(String.valueOf(prefs.getLong("hold_dur", 100L)));
        root.addView(etHoldDur);

        btnApply = btn("Apply & Restart Widget", 0xFF4CAF50);
        btnApply.setOnClickListener(v -> {
            UIHelper.showLoading(SettingsActivity.this, "Saving settings...");
        ...
        btnReset.setOnClickListener(v -> UIHelper.showResetConfirm(this, () -> {
            resetDefaults();
            UIHelper.toastSuccess(this, "Settings reset to defaults");
        }));
        ...
        btnSample.setOnClickListener(v -> {
            UIHelper.showLoading(this, "Creating sample...");
            new android.os.Handler().postDelayed(() -> {
                boolean ok = JsonScriptLoader.importScriptFromString(this,
                        JsonScriptLoader.generateSampleScript(), "sample.json");
                UIHelper.dismissLoading();
                if (ok) UIHelper.toastSuccess(this, "Sample script created");
                else UIHelper.toastError(this, "Failed to create sample");
            }, 400);
        });
    ...
    private void pickColor(boolean isDot) {
        ...
        new android.app.AlertDialog.Builder(this)
                ...
                .setItems(names, (d, i) -> {
                    if (isDot) {
                        dotColor = colors[i];
                        btnDotColor.setText("Dot Color: #" + Integer.toHexString(dotColor).toUpperCase());
                    } else {
                        ringColor = colors[i];
                        btnRingColor.setText("Ring Color: #" + Integer.toHexString(ringColor).toUpperCase());
                    }
                }).show();
    }
    ...
    private void applySettings() {
        ...
        UIHelper.toastSuccess(this, "Saved! Restart widget from main screen.");
    }

    private void resetDefaults() {
        sbDotSize.setProgress(22 - 4);
        sbRingSize.setProgress(50 - 10);
        sbRingThick.setProgress(2 - 1);
        sbDotAlpha.setProgress(220 - 10);
        sbRingAlpha.setProgress(20 - 1);
        dotColor = Color.RED;
        ringColor = Color.WHITE;
        btnDotColor.setText("Dot Color: #FF0000");
        btnRingColor.setText("Ring Color: #FFFFFF");
        etHoldStartX.setText("540");
        etHoldStartY.setText("1200");
        etHoldEndX.setText("540");
        etHoldEndY.setText("600");
        etHoldDur.setText("100");
    ...
    private void showScripts() {
        ...
        if (s.isEmpty()) {
            UIHelper.toastInfo(this, "No scripts found");
            return;
        }
        StringBuilder sb = new StringBuilder();
        for (String x : s) sb.append("• ").append(x).append("\n");
        UIHelper.showInfo(this, "Saved Scripts (" + s.size() + ")", sb.toString());
    }
    ...
    private SeekBar seek(int prog, int min, int max) {
        SeekBar sb = new SeekBar(this);
        sb.setMax(max - min);
        sb.setProgress(prog - min);
    ...
    private EditText edit(String val) {
        EditText e = new EditText(this);
        e.setText(val);
        e.setTextColor(0xFFFFFFFF);
        e.setBackgroundColor(0x22FFFFFF);
